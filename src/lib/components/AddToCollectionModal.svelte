<script lang="ts">
	...
	let { placeIds, label, collections, collectionPlacesMap, onToggle, onClose, anchorX, anchorY }: Props = $props();
	...
	let adjustedX = $derived(
		anchorX != null
			? Math.max(MARGIN, Math.min(anchorX, (typeof window !== 'undefined' ? window.innerWidth : 800) - POPOVER_W - MARGIN))
			: 0
	);
	let adjustedY = $derived(
		anchorY != null
			? Math.max(MARGIN, Math.min(anchorY, (typeof window !== 'undefined' ? window.innerHeight : 600) - POPOVER_MAX_H - MARGIN))
			: 0
	);

	let isPopover = $derived(anchorX != null && anchorY != null);
...
{#if isPopover}
	<!-- Popover mode (anchored near the trigger button) -->
	<button
	...
	<div
		class="fixed z-[60] w-72 overflow-hidden rounded-xl border border-warm-200 bg-white shadow-xl"
		style="left: {adjustedX}px; top: {adjustedY}px"
	>
		<div class="flex items-center justify-between border-b border-warm-100 px-3.5 py-2.5">
			<div class="min-w-0 flex-1">
				<span class="text-sm font-bold text-warm-800">Add to Collection</span>
				<p class="truncate text-xs text-warm-400">{label}</p>
			</div>
			<button onclick={onClose} class="shrink-0 rounded-md p-1 text-warm-400 hover:bg-warm-100 hover:text-warm-600" aria-label="Close">
			...
			{:else}
				{#each collections as col (col.id)}
					...
					<button
						...
						class="flex w-full items-center gap-3 px-3.5 py-2.5 text-left transition-colors hover:bg-warm-50"
					>
...
{:else}
	<!-- Fallback: centered modal (for cases without anchor, e.g. batch from saved views) -->
