<script lang="ts">
	...
	const cfg = $derived.by(() => {
		switch (size) {
			case 'xs':
				return { outer: 'h-7 w-7', emoji: 'text-base leading-none', dot: 'h-2.5 w-2.5', ring: 1.5 };
			case 'sm':
				return { outer: 'h-9 w-9', emoji: 'text-lg leading-none', dot: 'h-3 w-3', ring: 2 };
			case 'md':
				return { outer: 'h-10 w-10', emoji: 'text-xl leading-none', dot: 'h-3.5 w-3.5', ring: 2 };
			case 'lg':
				return {
					outer: 'h-8 w-8 sm:h-9 sm:w-9',
					emoji: 'text-base leading-none sm:text-lg',
			...
			case 'xl':
				return {
					...
					emoji: 'text-xl leading-none sm:text-2xl',
