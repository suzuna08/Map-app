...
export const load: PageServerLoad = async ({ locals }) => {
	...
	const [placesRes, tagsRes, placeTagsRes, listsRes, listPlacesRes] = await Promise.all([
		supabase.from('places').select('id, user_id, title, note, url, source_list, created_at, google_place_id, category, primary_type, rating, rating_count, price_level, address, area, description, lat, lng, phone, website, enriched_at, user_rating, user_rated_at').order('created_at', { ascending: false }),
